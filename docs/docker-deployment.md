...
When an HTTPS proxy is present, the build script enables the `RELAX_HTTPS_VERIFICATION=true` build arg by default so npm / pip can work behind self-signed interception proxies. You can override that behavior with `RELAX_HTTPS_VERIFICATION=false ./scripts/build_docker.sh`. If the shell also exposes a readable PEM CA file path (`BUILD_CA_CERT`, `NODE_EXTRA_CA_CERTS`, `NPM_CONFIG_CAFILE`, `PIP_CERT`, `REQUESTS_CA_BUNDLE`, `CURL_CA_BUNDLE`, or `SSL_CERT_FILE`), that certificate is mounted into the build as an additional fallback.
